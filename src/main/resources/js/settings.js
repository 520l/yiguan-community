...
var Settings = {
  ...
  initBag: function (sysBag) {
    ...
    if (bag.metalTicket !== undefined && bag.metalTicket > 0) {
      html += '<button style="margin:0 5px 5px 0" onclick="alert(\'您已取得一罐社区一周年纪念勋章领取权限，请静待系统公告\')">一罐社区一周年纪念勋章领取券 x' + bag.metalTicket + '</button>';
    }
  ...
  use1dayCheckinCard: function (csrfToken) {
    let confirmText = '使用单日免签卡后，明天的签到将由系统自动帮你完成，不需要登录一罐社区。确定使用吗？';
  ...
  use2dayCheckinCard: function (csrfToken) {
    let confirmText = '使用两天免签卡后，明天和后天的签到将由系统自动帮你完成，不需要登录一罐社区。确定使用吗？';
    if (Settings.remainCheckin > 0) {
      confirmText = '您已经有生效中的免签卡，如继续使用时间将继续叠加，确认继续吗？';
    }
    if (confirm(confirmText) === true) {
  ...
  requestDeactive: function (csrfToken) {
    if (confirm("请注意！这不是保存按钮！！！\n点击确定后，您的一罐社区账号将会被永久停用，无法登录，账户信息将被部分抹除，您绑定的手机号需要一个月后才能在一罐社区重新注册账号，确定继续吗？")) {
      if (confirm("亲爱的鱼油，再次向您确认！\n您的账号数据非常宝贵，如果对社区的发展有任何意见或建议，欢迎联系一罐社区管理组。\n本次确认后，您的账户将被永久停用。")) {
