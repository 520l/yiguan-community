...
<script>
    ...
    function randomPoem() {
        ...
        switch (num) {
            case 1:
                return "都放假了，有逛一罐社区的时间，出去玩一玩，它不香吗？";
                break;
            case 2:
...
<script>
    ...
    <#if userPhone == "">
        Util.alert("⛔ 为了确保账号的安全及正常使用，依照相关法规政策要求：<br>您需要绑定手机号后方可正常访问一罐社区。<br><br><button onclick='location.href=\"${servePath}/settings/account#bind-phone\"'>点击这里前往设置</button>")
    </#if>

    <#if need2fa == "yes">
    Util.alert("⛔ 一罐社区管理组成员，您好！<br>作为管理组的成员，您的账号需要更高的安全性，以确保社区的稳定运行。<br>请您收到此通知后，立即在个人设置-账户中启用两步验证，感谢你对社区的贡献！<br><br><button onclick='location.href=\"${servePath}/settings/account#mfaCode\"'>点击这里前往设置</button>", "致管理组成员的重要通知️")
    </#if>
</script>
<script>
    ...
    function goodNight() {
        $("#goodNight").html('' +
            ...
            '   <div style="font-size: 17px; margin-top: 9px">现在是一罐社区的休息时间，但小派还在默默地陪伴你，请尽快完成工作回家吧~ <span class="ft-red">♥️</span></div>' +
            '</div>');
        setTimeout(function () {
