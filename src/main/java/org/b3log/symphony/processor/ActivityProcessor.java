        ...
        try {
            ...
            try {
                ...
                if (ADRLimiter.access(currentUser.optString(User.USER_NAME))) {
                    ...
                    context.renderJSON(StatusCodes.SUCC);
                    context.renderMsg("数据上传成功，恭喜你通关了！你获得了奖励 " + amout + " 积分！你可以在一罐社区-总榜-ADarkRoom总分榜单中查看你的成绩！");
                } else {
                    context.renderJSON(StatusCodes.ERR);
                ...
                context.renderJSON(StatusCodes.ERR);
                context.renderMsg("存储数据失败！原因：你还没有登录一罐社区，请前往一罐社区 https://fishpi.cn 登录账号后重试。");
            }
        } catch (Exception e) {
            context.renderJSON(StatusCodes.ERR);
